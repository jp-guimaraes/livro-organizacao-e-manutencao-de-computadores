...
Os termos *arquitetura de computadores* e *organização de computadores* são, com frequência, usados como sinônimos — inclusive na literatura técnica e no vocabulário do mercado de tecnologia. Este livro, seguindo a distinção adotada por autores como Stallings, trata os dois termos como conceitos tecnicamente distintos.
...
[IMAGEM: comparativo de anúncios de processadores i3/i5/i7 de mesma geração, com núcleos e cache destacados]

## 5.2 A arquitetura de von Neumann formalizada

O Capítulo 1 introduziu o conceito de **programa armazenado**, formalizado por John von Neumann: a ideia de que instruções e dados residem na mesma memória do computador, permitindo que o programa seja executado de forma autônoma, sem intervenção humana a cada etapa.
...
Em 1981, a IBM lançou o IBM PC utilizando o processador Intel 8088, derivado do 8086 — um chip de 16 bits originalmente projetado para uso em calculadoras, escolhido por seu baixo custo (conforme apresentado no Capítulo 1). O sucesso comercial desse computador consolidou o conjunto de instruções do 8086 como padrão de mercado, e toda a evolução subsequente da família de processadores da Intel — e, mais tarde, da AMD — preservou esse conjunto de instruções original, apenas adicionando novas instruções a cada geração.
...
|---|---|
| 8086 | 16 bits; originalmente projetado para uma calculadora; base do computador que a IBM adotou |
...
| 80386 | Lançado em 1985; primeira geração de 32 bits da família, retrocompatível até o 8086 |
| 80486 | Evolução do 386 com memória cache integrada ao processador |
| Pentium | Sucessor do 486; a Intel abandonou a numeração "586" e passou a usar um nome comercial |
...
| Core Ultra / Core 5, 7, 9 | Nomenclatura adotada a partir de 2025, substituindo i3/i5/i7 após décadas de uso |
...
Ao conjunto de processadores que evoluem dessa forma — acumulando instruções ao longo do tempo, sem nunca descartar as anteriores — dá-se o nome de arquitetura **CISC** (*Complex Instruction Set Computer*, computador com conjunto de instruções complexo). O ponto forte dessa filosofia é justamente a retrocompatibilidade; o ponto fraco é o alto consumo energético, decorrente do acúmulo constante de complexidade no hardware ao longo de décadas de evolução.

[IMAGEM: diagrama de Venn com círculos concêntricos representando 8086 ⊂ 286 ⊂ 386 ⊂ 486 ⊂ Pentium ⊂ Core]
...
[IMAGEM: comparação de páginas de download de um mesmo programa, mostrando as versões disponíveis para x86, x64 e ARM]

## 5.5 RISC: o paradigma da simplicidade energética
...
A principal família comercial de processadores RISC de propósito geral é a **ARM**, presente em smartphones, no Raspberry Pi e, desde 2010, também em notebooks (como os MacBooks equipados com chips da série M — M1, M2, M3, M4, M5) e em processadores da linha Snapdragon.
...
[IMAGEM: fotos comparativas de um chip Cortex-A (Raspberry Pi), um Snapdragon (smartphone) e um ATmega328 (Arduino Uno)]
...
| Consumo energético | Mais alto | Mais baixo |
...
A adoção da arquitetura ARM tem se expandido para além de dispositivos móveis. O projeto **Asahi Linux**, em desenvolvimento desde o início da década de 2020, busca portar o sistema operacional Linux para funcionar de forma plena nos chips ARM da série M da Apple, originalmente destinados exclusivamente ao macOS.

Há também uma tendência discutida no mercado de tecnologia — e que o professor responsável por este curso considera provável, embora se trate de uma projeção, não de um fato consolidado — de migração gradual de servidores para arquitetura ARM, motivada pela economia de energia proporcionada pelo paradigma RISC: um servidor opera de forma contínua, e uma redução percentual no consumo energético, multiplicada pelo tempo de operação ininterrupta, representa economia financeira significativa em escala.

[IMAGEM: captura de tela de um site de hospedagem em nuvem mostrando opções de servidor com processador ARM ao lado de opções x86/x64]
...
**Tabelas de partição: MBR e GPT.** MBR (*Master Boot Record*) é o esquema de particionamento mais antigo, limitado a quatro partições primárias (contornável por meio de uma partição estendida contendo partições lógicas) e a discos de até aproximadamente 2 TB. GPT (*GUID Partition Table*) é o esquema mais recente, sem essa limitação prática de tamanho de disco, com suporte a um número muito maior de partições, rótulos (*labels*) nomeáveis e uma cópia de segurança da própria tabela de partição em outra região do disco. O uso de GPT depende de suporte do firmware da placa-mãe: computadores com BIOS tradicional (não UEFI) não têm suporte a GPT; computadores com UEFI podem trabalhar tanto com GPT quanto, por retrocompatibilidade, com MBR.
